    ...
    <div className="space-y-6">
      ...
      {leads.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
          <h3 className="text-sm font-semibold text-slate-900 mb-3">Lead Funnel</h3>
          <div className="space-y-2">
            {[
              { label: 'Total Leads', count: total, color: 'bg-slate-200' },
              { label: 'Contacted', count: contacted + hot, color: 'bg-amber-400' },
              { label: 'Hot', count: hot, color: 'bg-rose-400' },
              { label: 'Won', count: won, color: 'bg-emerald-500' },
            ].map((stage) => (
              <div key={stage.label} className="flex items-center gap-3">
                <div className="w-24 text-xs font-medium text-slate-600 text-right">{stage.label}</div>
                ...
                <div className="w-12 text-xs font-medium text-slate-700 tabular-nums">{stage.count}</div>
              </div>
            ))}
          </div>
        </div>
